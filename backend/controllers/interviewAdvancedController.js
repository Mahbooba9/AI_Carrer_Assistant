...
...`;
    ...
    const topicsResponse = await generateGroqContent([{ role: 'user', content: prompt }], 'llama3-8b-8192');
...
Make it detailed and beginner-friendly if Beginner level, intermediate if Intermediate, advanced if Advance. Ensure content is tailored to the ${role} role.`;
    ...
    const content = await generateGroqContent([{ role: 'user', content: prompt }], 'llama3-8b-8192');
...
[
  {
    ...
    const quizDataText = await generateGroqContent([{ role: 'user', content: prompt }], 'llama3-8b-8192');
    let questions = [];
